...
const MODULUS: usize = 1 << CHUNK_SIZE;
...
fn lut_8_to_4() {
    ...
    let msg_lower_extr = masked_bit_extraction(msg_lower);
    let mut msg_upper_extr = masked_bit_extraction(msg_upper);
    ...
    for (bit_idx, fourier_ggsw) in fourier_ggsw_list.as_view().into_ggsw_iter().enumerate() {
        ...
        cmux_assign(&mut accumulator, &mut buf, &fourier_ggsw);
    }
...
fn lut_16_to_4() {
    ...
    let modified_lut = (0..(1 << lut_input_size)).map(|i| {
        ...
        let masked_i_lhs = masking_chunk_msb(i_lhs);
        let masked_i_rhs = masking_chunk_msb(i_rhs);

        let masked_input = masked_i_lhs + (masked_i_rhs << MESSAGE_SIZE);
        lut[masked_input]
    }).collect::<Vec<usize>>();

    let lhs_lower_extr = masked_bit_extraction(lhs_lower);
    let mut lhs_upper_extr = masked_bit_extraction(lhs_upper);
    let mut rhs_lower_extr = masked_bit_extraction(rhs_lower);
    let mut rhs_upper_extr = masked_bit_extraction(rhs_upper);
...
fn masked_bit_extraction(input: usize) -> Vec<usize> {
...
fn masking_chunk_msb(input: usize) -> usize {
    assert!(input < (1 << MESSAGE_SIZE));

    let lower = input % MODULUS;
    let upper = input >> CHUNK_SIZE;

    let lower_extr = masked_bit_extraction(lower);
    let upper_extr = masked_bit_extraction(upper);
