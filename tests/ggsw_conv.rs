...
fn main() {
    ...
    let pfpks_base_log = DecompositionBaseLog(15);
    ...
    test_ggsw_conv_by_trace_with_preprocessing(
        lwe_dimension,
        glwe_dimension,
        polynomial_size,
        lwe_modular_std_dev,
...
fn test_ggsw_conv_by_pfpks(
    ...
    pfpks_base_log: DecompositionBaseLog,
    pfpks_level: DecompositionLevelCount,
    ...
    println!(
"==== GGSW conversion by private functional packing keyswitching ====
n: {}, N: {}, k: {},
l_pbs: {}, B_pbs: 2^{}, l_ggsw: {}, B_ggsw: 2^{}, LutCount: 2^{},
l_pfpks: {}, B_pfpks: 2^{}, l_ggsw: {}, B_ggsw: 2^{}\n",
        ...
        pfpks_level.0, pfpks_base_log.0, ggsw_level.0, ggsw_base_log.0,
    );
    ...
    let mut lev = LweCiphertextList::new(0u64, lwe_size, LweCiphertextCount(ggsw_level.0), ciphertext_modulus);

    let now = Instant::now();
    lwe_msb_bit_to_lev(&lwe_in, &mut lev, fourier_bsk, ggsw_base_log, ggsw_level, log_lut_count);
    let time = now.elapsed();
    println!("LWE to Lev: {} ms", time.as_micros() as f64 / 1000f64);
    ...
    println!("Lev to GGSW: {} ms, {:.2} bits", time.as_micros() as f64 / 1000f64, (max_err as f64).log2());
    ...
    let time = now.elapsed();
    println!("Fourier conversion: {} ms", time.as_micros() as f64 / 1000f64);
...
#[allow(unused)]
fn test_ggsw_conv_by_trace_with_preprocessing(
    lwe_dimension: LweDimension,
    glwe_dimension: GlweDimension,
    polynomial_size: PolynomialSize,
    lwe_modular_std_dev: impl DispersionParameter,
    ...
    println!(
"==== GGSW conversion by trace with preprocessing ====
n: {}, N: {}, k: {},
l_pbs: {}, B_pbs: 2^{}, l_ggsw: {}, B_ggsw: 2^{}, LutCount: 2^{},
l_auto: {}, B_auto: 2^{}, fft type: {:?}, l_ss: {}, B_ss: 2^{}\n",
    ...
    let now = Instant::now();
    let fourier_ggsw = circuit_bootstrap_lwe_ciphertext_by_trace_with_preprocessing(
        lwe_in.as_view(),
        fourier_bsk,
        &auto_keys,
        ss_key,
