...
use hom_trace::{mod_switch::*, automorphism::*, fast_automorphism::*};
...
fn main() {
    ...
    println!("param_message_2_carry_2_ks_pbs, fast");
    test_fast_trace_with_mod_switch(polynomial_size, glwe_dimension, glwe_modular_std_dev, auto_base_log, auto_level);
    println!();
...
fn test_fast_trace_with_mod_switch(
    polynomial_size: PolynomialSize,
    glwe_dimension: GlweDimension,
    glwe_modular_std_dev: StandardDev,
    auto_base_log: DecompositionBaseLog,
    ...
    let num_wramup = if polynomial_size.0 <= 2048 {100} else {10};
    for _ in 0..num_wramup {
        let _out = fast_trace(&ct, &fast_auto_keys);
    }
    ...
    // Fast Trace
    let now = Instant::now();
    let out = fast_trace(&ct_mod_up, &fast_auto_keys);
    let time_trace = now.elapsed();
