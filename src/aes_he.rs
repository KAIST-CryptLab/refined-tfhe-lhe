    ...
    OutputCont: ContainerMut<Element=Scalar>,
{
    ...
    let num_par_lut_per_sbox = polynomial_size.0 / (1 << BYTESIZE);
    let num_accumulator = if BYTESIZE % num_par_lut_per_sbox == 0 {
        BYTESIZE / num_par_lut_per_sbox
    } else {
        BYTESIZE / num_par_lut_per_sbox + 1
    };

    for acc_idx in 0..num_accumulator {
        // AES S-box
        let accumulator = (0..polynomial_size.0).map(|i| {
            let lut_idx = acc_idx * num_par_lut_per_sbox + i / (1 << BYTESIZE);
            (((AES128_SBOX[i % (1 << BYTESIZE)] & (1 << lut_idx)) as usize) << ((Scalar::BITS - 1) - lut_idx)).cast_into()
        }).collect::<Vec<Scalar>>();
        let accumulator_plaintext = PlaintextList::from_container(accumulator);
        let mut accumulator = allocate_and_trivially_encrypt_new_glwe_ciphertext(glwe_size, &accumulator_plaintext, ciphertext_modulus);
        ...
        for i in 0..num_par_lut_per_sbox {
            let bit_idx = acc_idx * num_par_lut_per_sbox + i;
            let mut lwe_out = output.get_mut(bit_idx);
            extract_lwe_sample_from_glwe_ciphertext(&accumulator, &mut lwe_out, MonomialDegree(i * (1 << BYTESIZE)));
        }
        ...
        let accumulator = (0..polynomial_size.0).map(|i| {
            let lut_idx = acc_idx * num_par_lut_per_sbox + i / (1 << BYTESIZE);
            (((AES128_SBOX_MULT_BY_2[i % (1 << BYTESIZE)] & (1 << lut_idx)) as usize) << ((Scalar::BITS - 1) - lut_idx)).cast_into()
        }).collect::<Vec<Scalar>>();
        let accumulator_plaintext = PlaintextList::from_container(accumulator);
        let mut accumulator = allocate_and_trivially_encrypt_new_glwe_ciphertext(glwe_size, &accumulator_plaintext, ciphertext_modulus);
        ...
        for i in 0..num_par_lut_per_sbox {
            let bit_idx = acc_idx * num_par_lut_per_sbox + i;
            let mut lwe_out = output_mult_by_2.get_mut(bit_idx);
            extract_lwe_sample_from_glwe_ciphertext(&accumulator, &mut lwe_out, MonomialDegree(i * (1 << BYTESIZE)));
        }
        ...
        let accumulator = (0..polynomial_size.0).map(|i| {
            let lut_idx = acc_idx * num_par_lut_per_sbox + i / (1 << BYTESIZE);
            (((AES128_SBOX_MULT_BY_3[i % (1 << BYTESIZE)] & (1 << lut_idx)) as usize) << ((Scalar::BITS - 1) - lut_idx)).cast_into()
        }).collect::<Vec<Scalar>>();
        let accumulator_plaintext = PlaintextList::from_container(accumulator);
        let mut accumulator = allocate_and_trivially_encrypt_new_glwe_ciphertext(glwe_size, &accumulator_plaintext, ciphertext_modulus);
        ...
        for i in 0..num_par_lut_per_sbox {
            let bit_idx = acc_idx * num_par_lut_per_sbox + i;
            let mut lwe_out = output_mult_by_3.get_mut(bit_idx);
            extract_lwe_sample_from_glwe_ciphertext(&accumulator, &mut lwe_out, MonomialDegree(i * (1 << BYTESIZE)));
        }
    }
}
