...
pub fn fourier_polynomial_torus_integer_mult_128<Scalar, ContOut, ContLhs, ContRhs>(
    out: &mut Polynomial<ContOut>,
    lhs_torus: &Polynomial<ContLhs>,
    rhs_int: &Polynomial<ContRhs>,
    modulus_sup: usize,
    ...
    ContRhs: Container<Element=Scalar>,
{
    ...
    let mut wrapper_fourier_ggsw_lhs = Fourier128GgswCiphertext::from_container(
        boxed_re0,
        boxed_re1,
        boxed_im0,
        boxed_im1,
    ...
    wrapper_fourier_ggsw_lhs.fill_with_forward_fourier(&wrapper_ggsw_lhs, fft);
...
pub fn fourier_glwe_polynomial_mult_128<Scalar, ContOut, ContLhs, ContRhs>(
    out: &mut GlweCiphertext<ContOut>,
    lhs: &GlweCiphertext<ContLhs>,
    rhs: &Polynomial<ContRhs>,
    modulus_sup: usize,
    ...
    ContRhs: Container<Element=Scalar>,
{
    ...
    out.as_mut().fill(Scalar::ZERO);
    for (mut out_poly, lhs_poly) in out.as_mut_polynomial_list().iter_mut().zip(lhs.as_polynomial_list().iter()) {
        fourier_polynomial_torus_integer_mult_128(&mut out_poly, &lhs_poly, rhs, modulus_sup);
    }
}
