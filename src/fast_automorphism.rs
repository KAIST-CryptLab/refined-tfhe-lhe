...
pub struct FastAutomorphKey<C: Container<Element=c64>> {
...
impl FastAutomorphKey<ABox<[c64]>> {
    pub fn allocate(
        decomp_base_log: DecompositionBaseLog,
        decomp_level_count: DecompositionLevelCount,
        glwe_dimension: GlweDimension,
    ...
    fn fill_with_fast_automorph_key<Scalar: UnsignedTorus + Sync + Send, G: ByteRandomGenerator>(
        &mut self,
        ...
        *before_key = GlweSecretKey::from_container(before_poly_list.into_container(), self.polynomial_size);
        ...
        self.fill_with_fast_keyswitch_key(before_key, after_key, noise_parameters, generator);
        self.auto_k = k;
    }
    ...
    fn fill_with_fast_keyswitch_key<Scalar: UnsignedTorus + Sync + Send, G: ByteRandomGenerator>(
        &mut self,
    ...
    fn fast_keyswitch_ciphertext<Scalar, InputCont, OutputCont>(
        &self,
    ...
    pub fn fast_auto<Scalar, InputCont, OutputCont>(
        &self,
        after: &mut GlweCiphertext<OutputCont>,
        before: &GlweCiphertext<InputCont>,
    ) where
        ...
        OutputCont: ContainerMut<Element=Scalar>,
    {
        ...
        self.fast_keyswitch_ciphertext(after, &before_power);
    }
}

pub fn gen_all_fast_auto_keys<Scalar, G>(
    decomp_base_log: DecompositionBaseLog,
    decomp_level: DecompositionLevelCount,
    glwe_secret_key: &GlweSecretKeyOwned<Scalar>,
    noise_parameters: impl DispersionParameter,
    generator: &mut EncryptionRandomGenerator<G>,
) -> HashMap<usize, FastAutomorphKey<ABox<[c64]>>>
where
    Scalar: UnsignedTorus + Sync + Send,
    G: ByteRandomGenerator,
{
    ...
    for i in 1..=(polynomial_size.0).ilog2() as usize {
        ...
        let mut glwe_ksk = FastAutomorphKey::allocate(decomp_base_log, decomp_level, glwe_dimension, polynomial_size, i);
        let mut before_key = glwe_secret_key.clone();

        glwe_ksk.fill_with_fast_automorph_key(&mut before_key, &glwe_secret_key, k, noise_parameters, generator);
...
pub fn fast_trace<Scalar, Cont>(
    glwe_in: &GlweCiphertext<Cont>,
    auto_keys: &HashMap<usize, FastAutomorphKey<ABox<[c64]>>>,
    ...
    Cont: Container<Element=Scalar>,
{
    ...
    fast_trace_assign(&mut out, auto_keys);

    out
}

pub fn fast_trace_assign<Scalar, ContMut>(
...
{
    fast_trace_partial_assign(glwe_in, auto_keys, 1);
}

pub fn fast_trace_partial_assign<Scalar, ContMut>(
    ...
    ContMut: ContainerMut<Element=Scalar>,
{
    ...
    for i in 1..=(log_polynomial_size - log_n) {
        ...
        auto_key.fast_auto(&mut buf, &out);
